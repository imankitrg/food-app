...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span
      className={`px-2.5 py-1 rounded-lg border text-xs font-medium capitalize ${styles[status] || styles.pending}`}
    >
      {status}
    </span>
  );
...
export default function AdminDashboard() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setOrdersError(data.msg || "Failed to fetch orders.");
        return;
      }
    ...
    <div className="min-h-screen bg-[#080808] text-white">
      ...
      <div className="border-b border-white/6 bg-[#0d0d0d] sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-xl bg-orange-500 flex items-center justify-center text-sm font-bold">
              F
            </div>
            <div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-6">
        ...
        <div className="flex items-center justify-between mb-5">
          ...
          >
            <span className={ordersLoading ? "animate-spin" : ""}>
              <IconRefresh />
            </span>
            Refresh
          </button>
        ...
        {ordersLoading && orders.length === 0 && (
          <div className="space-y-2">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="bg-[#111] border border-white/6 rounded-xl px-4 py-3.5 flex items-center gap-4 animate-pulse"
              >
        ...
        <div className="space-y-2">
          {filteredOrders.map((order) => (
            <div
              key={order._id}
              className="bg-[#111] border border-white/6 hover:border-white/10 rounded-xl overflow-hidden transition-all"
            >
              {/* Row */}
              <div
              ...
              >
                ...
                <p className="text-sm font-semibold text-white flex-shrink-0 w-16 text-right">
                  ₹{order.totalAmount}
                </p>
                ...
                <div
                  className={`text-zinc-600 transition-transform flex-shrink-0 ${expandedOrder === order._id ? "rotate-180" : ""}`}
                >
                  <IconChevron />
                </div>
              </div>
